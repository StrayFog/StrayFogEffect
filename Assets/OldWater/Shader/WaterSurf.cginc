#pragma target 4.6
#include "CameraDepthTexture.cginc"
#include "River.cginc"
//Water
...
void tessSurf(Input IN, inout SurfaceOutputStandardSpecular o) {
	...
	float3 worldReflect = reflect(-worldLightDir, worldNormal);

	//specular
	half specularFactor = max(dot(worldReflect, worldView), 0);

	//ambient color
	float3 ambientColor= UNITY_LIGHTMODEL_AMBIENT.rgb;

	//lambert
	//half lambertFactor = max(dot(worldNormal, worldLightDir), 0) * 0.5 + 0.5;
	//lambert
	half lambertFactor = max(dot(worldNormal, worldLightDir), 0);


	fixed fresnel = 0.5 + (1 - 0.5) * pow(1 - dot(worldView, worldNormal), 5);
